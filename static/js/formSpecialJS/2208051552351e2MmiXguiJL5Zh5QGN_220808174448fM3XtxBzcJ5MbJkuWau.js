  ...
  exportObj.initHandle = function (formData, formStruct, vueData) {
    ...
    dsf.http.post("fn/superviseV6/approvalTask/hasTasks", { relationId: vueData.$route.query.sourcePk || formData.main["A0031"].value }).then(res => {
      ...
      if (res.data.code == 200 && res.data.data && res.data.data.taskTitle) {
        ...
        vueData.$set(formData.main, "C-DB-CHANGE-0013", { value: res.data.data.taskId })
      }
    ...
    let specialSaveObj = JSON.stringify({ relationId: vueData.$route.query.sourcePk || formData.main["A0031"].value })
    let NewLinkInfo = `/commonForm/210604144527JntE2LI7dWrpBLsluZz?specialSaveObj=${specialSaveObj}&fid=${vueData.formMeta.parameters.pk}`
    // if (formStruct.parameters.nodeId != 0) {
    try {
      dsf.http
        .post("fn/superviseV6/approvalTask/hasTasksByFid", {
          relationId: vueData.formMeta.parameters.pk
        })
        .then(res => {
          if (res.data.code == 200 && res.data.data && res.data.data.taskTitle) {
            ...
            vueData.$set(formData.main, "C-DB-CHANGE-0014", { value: res.data.data.taskId })
          }
