;(function () {
  ...
  exportObj.initHandle = function (formData, formStruct, vueData) {
    ...
    if (formStruct.parameters.nodeId === "XVFQiYfrCLfubcN") {
      ...
      let enumData = DB_0081.enumData || []
      DB_0081.enumData = enumData.filter(item => item.value == 6 || item.value == 1)
      // for (let i = 0; i < enumData.length; i++) {
      //     if ($(enumData[i]).find("input").val() != '6'){
      //         $(enumData[i]).hide();
      //     }
      // }
    }
    ...
    vueData.$bus.on("inputRadio", function (busObj) {
      ...
      if (busObj.metaData.metaId == "C-DB-0078") {
        ...
        let bmnbl = formStruct.controls.find(ctr => ctr.metaId == 'C-DB-0122')
        if (busObj.value == 5 || busObj.value == 6) {
          if (bmnbl) {
            vueData.$set(bmnbl.validator, "required", {})
          }
        } else {
          if (bmnbl) {
            vueData.$delete(bmnbl.validator, "required")
          }
        }
    ...
    vueData.$bus.$off("userClickBus").on("userClickBus", function (busObj) {
      console.log(busObj, "点击")
      if (busObj.metaId == "C-DB-0122") {
        if (busObj.user.id.split('_')[1] == dsf.util.loadLocalStore('user').user_id) {
          dsf.layer.toast('不能选择本人')
          busObj.user.checked = false
        }
      }
    })
