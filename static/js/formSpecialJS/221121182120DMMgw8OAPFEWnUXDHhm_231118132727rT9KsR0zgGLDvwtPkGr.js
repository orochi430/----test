	...
	exportObj.initHandle = function(formData, formStruct, vueData) {
		...
		dsf.http.post('ctrl/meetingRoomOccupy/getData', {
			...
			if (res.data.code == '200') {
				...
				vueData.$set(formData.main, "c-meetingsqbg-044", { value: data.gzryValue, text: data.gzryText, schema: data.gzrySchema }); //原部门通知人
				vueData.$set(formData.main, "c-meetingsqbg-045", { value: data.gzryValue, text: data.gzryText, schema: data.gzrySchema }); //现部门通知人
			}
	
		})
		vueData.$bus.on("selectUserBus", function (data) {
			console.log(data);
			if (data.metaData.metaId == "c-meetingsqbg-017") {
			  getDeptAndUser(data.value, 2)
			}
			if (data.metaData.metaId == "c-meetingsqbg-045") {
			  getDeptAndUser(data.value, 1)
			}
		});
		function getDeptAndUser(id, type){
			if (!id) {
				if (type == '1') {
				  vueData.$set(formData.main, "c-meetingsqbg-017", {
					value: '',
					text: '',
					schema: '',
				  });
				} else{
				  vueData.$set(formData.main, "c-meetingsqbg-045", {
					value: '',
					text: '',
					schema: '',
				  });
