...
;(function () {
  ...
  exportObj.initHandle = function (formData, formStruct, vueData) {
    ...
    vueData.$bus.on("inputRadio", function (busObj) {
      ...
      } else if (busObj.metaData.metaId == "C-DB-0106") {
        // 选择建议回复形式后生成办理要求
        let content = ''
        if (busObj.value == 1) {
            content = "请主办处室于办理时间节点前完成发文流程，并将成文报委办备案。";
        } else if (busObj.value == 2) {
            content = "请主办处室于办理时间节点前根据批示要求，将工作情况签报委领导，并将签报报委办备案。";
        } else if (busObj.value == 3) {
            content = "请主办处室于办理时间节点前反馈办理情况的纸质版和电子版。";
        }
        vueData.$set(formData.main, 'B0050', {value: content})
      }
    })
    // 选择时间节点后生成星期
    vueData.$bus.on("selectDateTime", function (busObj) {
      if (busObj.metaData.metaId == 'C-DB-0052') {
        vueData.$set(formData.main, 'C-DB-0018', {value: moment()._locale.weekdays()[new Date(busObj.value).getDay()]});
      }
    })
    // 选择时间节点后生成星期
    vueData.$bus.on("relationListAdd", function (busObj) {
      let showRows = busObj.list
      if (showRows && showRows.length > 0) {
        let relaId = showRows[0].dataValue.relaId;
        dsf.http.post("ctrl/sjwcustom/lw/get", {"pk": relaId}).then(res => {
            if (res.data && res.data.type == "success" && res.data.data) {
                // 来文单位
                vueData.$set(formData.main, 'C-DB-0109', {value: res.data.data.lwdw})
                // 收文编号
                vueData.$set(formData.main, 'C-DB-0056', {value: res.data.data.wh})
                // 来文文号
                vueData.$set(formData.main, 'C-DB-0098', {value: res.data.data.swh})
                // 文件名称
                vueData.$set(formData.main, 'C-DB-0107', {value: res.data.data.bt})
            }
        });
    }
    })
