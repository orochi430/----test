;(function () {
  ...
  function setFKQS(relationId, vueData) {
    dsf.http.post("ctrl/superviseV6/approval/getSuperviseInfo", { pk: relationId }).then(res => {
  ...
  exportObj.initHandle = function (formData, formStruct, vueData) {
    // 是否部门内反馈元数据
    vueData.$set(formData.main, "C-DB-FK-0051", { value: "1", text: "是" })
    // 获取项目名称
    ...
    // 关联立项单
    if (!vueData.relationList[0] || (!vueData.relationList[0].document.length && formStruct.controls.find(ctr => ctr.metaId.indexOf("relationFile") > -1))) {
              ...
              if (res.data.code == 200) {
                dsf.http
                  ...
                  .then(response => {
                    ...
                    formStruct.controls.find(ctr => ctr.metaId.indexOf("relationFile") > -1).show = 1
                  })
              }
            })
        })
    ...
    if (!vueData.formData.main["C-DB-FK-0024"].text) {
      formStruct.controls.forEach(ele => {
        if (ele.metaId == "C-DB-FK-0024") {
          if (ele.enumData.find(it => it.value == "1")) {
            vueData.$set(vueData.formData.main, "C-DB-FK-0024", {
              value: 1,
              text: "正常反馈"
            })
          } else if (ele.enumData.find(it => it.value == "3")) {
            vueData.$set(vueData.formData.main, "C-DB-FK-0024", {
              value: 3,
              text: "办结反馈"
            })
