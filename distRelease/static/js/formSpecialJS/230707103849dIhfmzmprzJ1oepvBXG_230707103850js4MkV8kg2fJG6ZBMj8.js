  ...
  let timeConfig = [];
    ...
    exportObj.initHandle = function (formData, formStruct, vueData) {
      ...
      vueData.$bus.on("checkVal", function (busObj) {
        if (busObj.metaData.metaId == "C-LDRC-0003") {
          ...
          if (config) {
            vueData.$set(formData.main, 'C-LDRC-0002', {value: `${config.currentDay}${config.startTime}`})
            vueData.$set(formData.main, 'C-LDRC-0008', {value: `${config.currentDay}${config.endTime}`})
          }
          ...
          if (last_C_LDRC_0001.split(',').length !== busObj.metaData.enumData.length && busObj.value.split(',').pop() == 'all') {
            ...
            vueData.$set(formData.main, 'C-LDRC-0001', obj)
    ...
    exportObj.saveFilter = function (formData, formStruct, vueData) {
      return new Promise(function (resolve, reject) {
        if (formData.main['C-LDRC-0001'].value.includes('all')) {
          formData.main['C-LDRC-0001'].text = formData.main['C-LDRC-0001'].text.split(',').filter(item => item !='全选').join(',')
        }
        resolve()
      })
    }
    ...
    exportObj.publish = function (formData, formStruct, vueData) {
      function func() {
        dsf.http.post('/fn/leader/publish', { 'pk': vueData.pk }).then(res => {
