(function () {
   ...
   vueData.$bus.on("selectUserBus", function (busObj) {
    ...
    vueData.$bus.on("selectUserBus", function (busObj, data) {
      if (busObj.metaData.metaId == "c-meetingtz-071") {
        ...
        if (PersonId == busObj.value) {
          vueData.$set(formData.main, "c-meetingtz-071", {
            value: "",
            text: "",
            schema: "",
          });
          dsf.layer.toast("委托人员不可选择自己，请重新选择");
    ...
    vueData.$bus.on("checkVal", function (data) {
      console.log(data)
      if (data.metaData.metaId == "B0058") {
        if (data.value == '0') {
          vueData.$set(formData.main["c-meetingtz-029"], "value", ''); //请假理由
        }
      }
    });
    ...
    function RenderList() {
      ...
      if (formData.main["c-meetingtz-057"] && formData.main["c-meetingtz-057"].value) {
  ...
  exportObj.saveFilter = function (formData, formStruct, vueData) {
    if (
      formData.multinfos["230517233506EpGpORnl6sDUjcR25a1"].rows.length == 0
    ) {
