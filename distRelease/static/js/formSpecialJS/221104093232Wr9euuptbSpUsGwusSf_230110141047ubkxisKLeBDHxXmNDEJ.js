...
(function () {
  ...
  exportObj.initHandle = function (formData, formStruct, vueData) {
    ...
    //   }
    // }
    if ("Gu6TDlmxyUVyhQw" == nodeId) {
      node = nodeId
      hideCloseButton(formStruct);
    }
  ...
  function hideCloseButton(formStruct) {
    let bt = formStruct.buttons.mainButtons;
    let pkId = formStruct.parameters.pk;
    let param = {
      "pkId": pkId
    };
    //请求后端查看是否转协同和有盖章文件 sums 为盖章文件个数， ifZxt 是否转协同， xtStatusCou 转协同后文件的状态
    dsf.http.post("ctrl/dealZxtContro/selectSenXt", param).then(result => {
      if (result.data.type == "success") {
        let sums = result.data.data.sums;
        let ifZxt = result.data.data.ifZxt;
        let xtStatusCou = result.data.data.xtStatusCou;

        if (sums != 1) {
          deleteBj(bt)
        } else if (ifZxt != -1 && sums == 1 && xtStatusCou != 1) {
          deleteBj(bt)
        }

        function deleteBj(bt) {
          for (let index in bt) {
            if (bt[index].name == "办结") {
              bt.splice(index, 1)
            }
