...
(function () {
  ...
  exportObj.initHandle = function (formData, formStruct, vueData) {
    ...
    vueData.$bus.on("selectUserBus", function (data) {
      ...
      if (data.metaData.metaId == "c-meetingtz-028") {
        getDeptAndUser(data.value, 2) // 出席部门获取部门通知人
      }
      if (data.metaData.metaId == "230801193302UOo9QBBNnmtzEOl8ag3") {
        getDeptAndUser(data.value, 1) // 部门通知人获取出席部门
      }
    });

    function getDeptAndUser(id, type){
      if (!id) {
        if (type == '1') {
          ...
          vueData.$set(formData.main, "230801193302UOo9QBBNnmtzEOl8ag3", {
            value: '',
            text: '',
            schema: '',
          });
      ...
      dsf.http
      .post("/fn/meetingNoticeJyj/userDeptConvert", {
        ids: id,
        type: type
      })
      .then((res) => {
                ...
                vueData.$set(formData.main, "c-meetingtz-020", {
                  value: formatMsToDate(
                    Number(data.duration * 3600000) +
                      Number(
                        new Date(
                          formData.main["c-meetingtz-019"].value.replace(
                            /-/g,
                            "/"
                          )
                        ).getTime()
      ...
      dsf.http
        ...
        .then((res) => {
          ...
          if (res.data.code == 200) {
            ...
            //默认会议室
            if (data.defaultMeetingRoom) {
              vueData.$set(formData.main, "c-meetingtz-010", {
                value: data.defaultMeetingRoom,
                text: data.defaultMeetingRoomText,
              });
            }else{
              vueData.$set(formData.main, "c-meetingtz-010", {
                value: '',
    ...
    vueData.$bus.$off("userClickBus").on("userClickBus", function (busObj) {
      ...
      console.log(busObj);
      if (
        busObj.metaId == "c-meetingtz-028" ||
        busObj.metaId == "c-meetingtz-041" ||
        busObj.metaId == "c-meetingtz-083" ||
        busObj.metaId == "c-meetingtz-084"
      ) {
        if (busObj.user.attrs.noselect) {
          ...
          checkDeptUser(ids).catch(() => {
            busObj.user.open = busObj.user.open == "" ? true : busObj.user.open;
            busObj.callback2(busObj.user, busObj.user.open);
          });
        } else {
    ...
    vueData.$bus
      .$off("usercheckAllBus")
      .on("usercheckAllBus", function (busObj) {
        return;
        console.log(busObj);
        if (
          (busObj.metaId == "c-meetingtz-028" ||
            ...
            busObj.metaId == "c-meetingtz-083" ||
            busObj.metaId == "c-meetingtz-084") &&
          busObj.user.children &&
          busObj.user.children.length > 0
        ) {
          if (!busObj.user.children[0].checked) {
            let ids;
            busObj.user.children.forEach((element, i) => {
              element.checked = false;
              if (i == 0) ids += element.attrs.idValue;
    ...
    function checkDeptUser(deptIds) {
      ...
      return new Promise((resolve, reject) => {
        dsf.http
          .post("ctrl/meetingRole/validDeptRoleUserNum", {
            deptIds: deptIds,
