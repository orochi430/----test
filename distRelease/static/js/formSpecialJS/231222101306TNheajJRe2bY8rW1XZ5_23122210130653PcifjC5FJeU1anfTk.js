...
(function () {
  ...
  exportObj.initHandle = function (formData, formStruct, vueData) {
    ...
    function checkInventory() {
      let status = formData.main["A0009"].value;
      let nodeId = formStruct.parameters.nodeId;
      let nodeflag = true;
      if ((nodeId === '0' || nodeId === '3MjcJYoN3NbRr4S' || nodeId === '-2' || nodeId === "kTG0UFn4hWSlMcZ") && status !== "2" && nodeflag) {
                ...
                sub.forEach(function (item) {
                  ...
                  if (item.action !== 'Deleted') {
                    if (item["C-OFFICESUPPLIES-RECEIVE-GOODS-0001"].value === key) {
                      let kc = data[key]["stockCount"];
                      vueData.$set(item, "C-OFFICESUPPLIES-RECEIVE-GOODS-0006", { "value": kc });//设置库存
                      let value = item["C-OFFICESUPPLIES-RECEIVE-GOODS-0007"].value;//领用数量
                      let regu = /^[1-9]+[0-9]*$/
                      if (value !== '') {
                        if (!regu.test(value)) {
