 ...
 */
 ...
 (function () {
  ...
  exportObj.initHandle = function (formData, formStruct, vueData) {
    ...
    // 初始化立项人电话
    if (!formData.main["C-DB-0126"] ||!formData.main["C-DB-0126"].value) {
  ...
  exportObj.saveFilter = function (formData, formStruct, vueData, action) {
    return new Promise(function (resolve, reject) {
      if (formData.multinfos["2208081707395f0kQgjx9XflsmUXkxT"] && formData.multinfos["2208081707395f0kQgjx9XflsmUXkxT"].rows.filter(row => row.action != 'Deleted').length > 0 || formData.main['C-DB-0001'].value) {
        resolve()
      } else{
        reject("请选择主办单位")
      }
