...
(function () {
  ...
  exportObj.initHandle = function (formData, formStruct, vueData) {
    ...
    vueData.$bus.on("selectUserBus", function (busObj) {
      if (busObj.metaData.metaId == "231221184233bflob5ZM3ESTCMntrOy") {
        dsf.http.post("ctrl/getUserInfo/orgAndUnit", {
          userId: busObj.value
        }).then(function (result) {
          if (result.data.type === "success") {
            let org = result.data.data[0];
            vueData.$set(formData.main, "B0051", { value: org.id, text: org.text, schema: org.schema })
            vueData.$set(formData.main, "B0011", { value: result.data.data[1].text })
          }
        });
      }
    })
    function checkInventory() {
      ...
      var nodeflag = true;
      // if (nodeId === "neZrAumhshj2bQJ") {
      //   if (formData.main["A0004"].value === "2312211841424jiWfKiwTG4110VkKlM") {
      //     nodeflag = false;
      //   }
      // }
      if ((nodeId === '0' || nodeId === '3MjcJYoN3NbRr4S' || nodeId === '-2' || nodeId === "neZrAumhshj2bQJ") && status !== "2" && nodeflag) {
