...
(function () {
    ...
    exportObj.link = function (listid1, moduleId1, pk1, vueData, itemValue) {
        ...
        let str = itemValue["UNIFY-DATA-0021.value"];
        ...
        //  .split("?")[1]
        // .split("=")[1]
        distribId = str.split("distribId")[1]
            .split("=")[1];
        type = str.split("type")[1]
            .split("=")[1];
        let userid = dsf.util.loadSessionStore("user").user_id;
        // dsf.base64.encode;
        let tmpUrl = dsf.base64.encode(
            `/commonForm/${moduleId}/${pk}?type=${type}=${distribId}&LocalPdfPreview=1`
        );
        let prefix = itemValue["UNIFY-DATA-0021.value"]
            .split("#")[0]
            .split("/dist")[0];
