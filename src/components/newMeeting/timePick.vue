...
export default {
    ...
    methods: {
        ...
        initData(cur) {
            ...
            // 切换月份自动切换日期--默认选中第一天
            if (moment(new Date()) < moment(this.weeks[0].date) || moment(new Date()) > moment(this.weeks[this.weeks.length - 1].date)) {
                this.weeks[0].isActive = true
            } else {
            ...
            let starData = moment(new Date(this.weeks[0].date)).format("YYYY-MM-DD");
            let endData = moment(new Date(this.weeks[this.weeks.length - 1].date)).format("YYYY-MM-DD");

            if (!this.able) {
                // 日查看时，初始化查看当天
                if (moment(new Date()) > moment(this.weeks[0].date) && moment(new Date()) < moment(this.weeks[this.weeks.length - 1].date)){
                    // 若切换的是当天所在星期，默认加载当天，非当天所在星期，默认加载星期一
                    this.$emit('onConfirm', moment(new Date()).format('YYYY-MM-DD'))
                }else{
                    this.$emit('onConfirm', starData)
                }
                
            }else{
                // 周查看时，查看当前周
                this.$emit('onConfirm', starData, endData)
            }
