...
export default {
    ...
    methods: {
        ...
        getScheduleData() {
            ...
            dsf.http.get("ctrl/p2433JxwLeaderSchedule/get", {
        ...
        saveSchedule(obj, callback) {
            ...
            if (obj) {
                params.visibleUser = '';
                params.visibleUserText = '';
                params.visibleUserSchema = '';
            }

            // if (that.scheduleData.status) {
            //     that.$toast.fail("该活动已经发布，请取消发布后编辑！");
            // } else {
            dsf.http.post("ctrl/p2433JxwLeaderSchedule/save", params, {
                    headers: {
        ...
        releaseSchedule(status) {
            ...
            if (status != 1) { //发布
                ...
                that.saveSchedule(obj, () => {
                    dsf.http.get("ctrl/p2433JxwLeaderSchedule/release", {
                            date: that.currentDate
                        })
                        .then(res => {
                            if (res.data.code == 200) {
                ...
                dsf.http.get("ctrl/p2433JxwLeaderSchedule/unRelease", {
                        date: that.currentDate
                    })
                    .then(res => {
                        if (res.data.code == 200) {
        ...
        getSchedule(startDate, endDate) {
            return new Promise((resolve, reject) => {
                dsf.http
                    .get("ctrl/p2433JxwLeaderSchedule/check", {
                        startTime: startDate,
                        endTime: endDate,
                    })
                    .then(res => {
