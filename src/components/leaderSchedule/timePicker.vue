<template>
  ...
  <div class="week-slider">
    <div class="slider-title" v-show="isShowMonth">
      <i class="icon iconfont iconsanjiaoyou iconsanjiaozuo" :class="{ 'title-icon-active': true }" @click="prevClick()"></i>
      <span style="font-weight: bold" @click="isShowSliderContent = true">{{ getYearMonth }}</span>
      <i class="icon iconfont iconsanjiaoyou" :class="{ 'title-icon-active': true }" @click="nextClick()"></i>
      <div class="right-icon">
        <span v-if="positionToday" @click="toToday()">今日</span>
        <slot name="right-icon"></slot>
      </div>
    </div>
    <collapseTransition>
      <div class="slider-content" v-show="isShowSliderContent">
        <van-swipe
                ...
                <div v-for="(item, inx) in base" :key="inx" class="day-of-week">
                  ...
                  <div
                    :class="{
                      ...
                      toDay: isToday(item.date) && !dsf.config.homePage.schedule.defaultToday,
                  ...
                  >
                    ...
                    <span
                      ...
                      :style="{
                        background: addSch(item.date) && judgeChecked(item.date) ? todayDotcolor : ''
                      }"
                      :class="{
                  ...
                  <span v-text="getLunarCalendar(item.date)" class="lunar-calendar" :class="[isHoliday(item.date) ? 'holiday' : '']"></span>
                </div>
              </div>
            </div>
          </van-swipe-item>
        ...
        <div v-show="defaultShowType == 'month'" class="date-box">
          <div class="lang-box" v-for="(item, index) in currentMonth" :key="index">
            <div v-for="(itm, inx) in item" :key="inx" class="day-of-week">
              ...
              <div :class="[itm.isCurrentMonth ? '' : 'not-current-month']">
                <div
                  ...
                  class="day-date"
                  @click="dateClick(itm.date)"
                >
                  <span>{{ itm.date.split("-")[2].replace(/^0+/, "") }}</span>
                  <span
                    v-show="addSch(itm.date)"
                    :style="{
                      background: addSch(itm.date) && judgeChecked(itm.date) ? todayDotcolor : ''
                    }"
                    :class="{
                ...
                <span v-text="getLunarCalendar(itm.date)" class="lunar-calendar" :class="[isHoliday(itm.date) ? 'holiday' : '']"></span>
              </div>
            </div>
          </div>
        </div>
        <div class="flex ac jc arrow-container">
          <van-icon v-if="defaultShowType == 'week'" name="arrow-down" @click="toggleShowType('month', true)" />
          <van-icon v-else name="arrow-up" @click="toggleShowType('week', true)" />
        </div>
      </div>
    </collapseTransition>
  </div>
  ...
  methods: {
    ...
    initTimer(date) {
      ...
      for (var key in this.weekLanguages) {
        this.weekLanguages[key] = this.weekLanguages[key].slice(self.startNumber - 1).concat(this.weekLanguages[key].slice(0, self.startNumber - 1))
      }
    ...
    getWeekInfo(date) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        var _formatDate = moment(date).subtract(self.weekLanguages.pos.indexOf(dateOfWeek) - i, "d")
        week.push({
          date: _formatDate.format("YYYY-MM-DD"), //日期
          isToday: today.format("YYYY-MM-DD") === _formatDate.format("YYYY-MM-DD"), //是否是今天
    ...
    dateClick(date) {
      if (date) {
        ...
        this.checkedDate = moment().format("YYYY-MM-DD")
      }
      //抛出点击的日期
    ...
    prevClick() {
      if (this.clickMode == "week") {
        ...
        this.reWriter = moment([this.curyear, this.curmonth, 1]).startOf("month").subtract("month", 1).format("YYYY-MM-DD")
        this.prev_now_next = []
    ...
    toToday() {
      if (!this.prev_now_next[1].find(date => date.date == moment().format('YYYY-MM-DD'))) {
        this.toCurrentWeek()
      }
      this.dateClick()
      this.isShowSliderContent = true
    },
    toCurrentWeek() {
      this.prev_now_next = []
      this.initTimer(moment().format('YYYY-MM-DD'))
    },
    //轮播完后触发的方法
    onChange(index) {
      this.reWriter = ""
      ...
      if (index == 0) {
        //滑动到最左边的时候
        this.prev_now_next.unshift(self.getWeekInfo(moment(self.prev_now_next[index][0].date).subtract(7, "d").format("YYYY-MM-DD")))
        this.prev_now_next.pop()
      } else if (index == 2) {
        //滑动到最右边的时候
        this.prev_now_next.push(self.getWeekInfo(moment(self.prev_now_next[index][0].date).add(7, "d").format("YYYY-MM-DD")))
    ...
    getFullMonth(date) {
      ...
      if (currentMonth[0].week != "一" && currentMonth[0].week != "Mon") {
        let lastMonth = this.getMonthInfo(moment(date).subtract(1, "months").format("YYYY-MM-DD"), false).reverse()
        for (let m = 0; m <= lastMonth.length; m++) {
          currentMonth.unshift(lastMonth[m])
          if (lastMonth[m].week == "一" || lastMonth[m].week == "Mon") break
        }
      }
      if (currentMonth[currentMonth.length - 1].week != "日" && currentMonth[currentMonth.length - 1].week != "Sun") {
        let nextMonth = this.getMonthInfo(moment(date).add(1, "months").format("YYYY-MM-DD"), false)
    ...
    getLunarCalendar(date) {
      ...
      return lunar.lunarFestival || lunar.festival || lunar.Term || (lunar.IDayCn == "初一" ? lunar.IMonthCn : lunar.IDayCn)
    },
    //获取节假日信息
    getHoliday(now, num) {
      let self = this
        ...
        dsf.http
          ...
          .then(res => {
            if (res.data.code == 200) {
              ...
              const map = new Map()
              self.holidayMonth = self.holidayMonth.filter(v => !map.has(v.date) && map.set(v.date, 1))
            } else {
