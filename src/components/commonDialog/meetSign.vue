...
export default {
    ...
    submit() {
      ...
      dsf.http
        .post("ctrl/meetingNotice/directSignUp", {
          bmId: this.metaData.parameters.bmId || this.$route.query.signUpPk || this.metaData.parameters.pId || this.metaData.parameters.todoId.split('_')[0],
          tzId: this.metaData.parameters.tzId|| this.formParams.pk,
          registrationStatus: "1",
        })
        .then((res) => {
          if (res.data.code == 200) {
            dsf.layer.toast("报名提交成功", ()=>{
              that.show0 = false;
  ...
  mounted() {
    if (this.formData) {
      this.value = this.formData.main["B0001"].value;
    }
  },
  destroyed() {},
};
