<template>
  ...
  <div style="width: 100%; height: 100%" class="dsf-tree">
    ...
    >
      <van-swipe-cell v-if="!!item.groupId">
        <div class="flex-box " :class="{ treeMargin: item.deep == 1,tree_line: item.isopen&&item.deep == 1}">
          <!-- 父级是否可选 -->
          <van-checkbox
            icon-size="18px"
            :style="getStyle(item.deep)"
            v-if="!item.attrs.noselect && item.attrs.isDirectory != 1"
          ...
          >
            <div class="tree_tit" :class="{ w_normal: !item.attrs.noselect && item.attrs.isDirectory != 1 }">
              {{ item.showName || item.name }}
              
            </div>
          </van-checkbox>
          <div
            class="tree_tit w_normal"
            v-else
            @click="usercheckAll(item, true)"
            :style="getStyle(item.deep)"
          >
            {{ item.showName || item.name }}
            <span v-if="!dsf.config.commonForm.isHideControlTreeNum && (item.attrs.childrenCount > 0 || item.personChildrenLen > 0)">
                        ({{ item.attrs.childrenCount > 0 ? item.attrs.childrenCount : item.personChildrenLen }})
                    </span>
          </div>
          ...
          <div
            class="isShowChiled"
            @click="checkAll(item)"
            v-if="item.attrs.childrenCount > 0 || item.children?.length > 0"
          >
      ...
      <div class="flex-box" v-if="!item.groupId" :class="{ treeMargin: item.deep == 1,tree_line: item.isopen&&item.deep == 1}">
        ...
        <i v-if="item.pid != 'codeGroupPrivate'&&item.id != 'codeGroupPrivate'&&extra.allowEdit&&item.children?.length > 0"  class="icon iconfont icontianjia" @click="showManual(item)" />
        <div
          class="isShowChiled"
          @click="checkAll(item)"
          v-if="item.attrs.childrenCount > 0 || item.children?.length > 0"
      ...
      >
        <div
          v-if="cell.type == 1 || !cell.children"
          :style="getStyle(item.deep + 1)"
          class="person"
        >
          <div class="flex-box person_child">
            ...
            >
              ...
              <div class="user-name">{{ cell.showName || cell.name }}{{currentTab.key=="canonical_group-public-private" ? `【${cell.attrs.unitName}/${cell.attrs.deptName}】` : ''}}</div>
            </van-checkbox>
          </div>
        </div>
        <div v-else>
...
export default {
  ...
  methods: {
        ...
        if (
          this.selectper.find(
            (select) =>
              ...
              ((select.type == 1 && select.pid.indexOf(pid) > -1) ||
                select.type != 1)
          )
        ) {
          item.checked = true;
          ...
          if (!this.personAddList_.find((it) => it.id.indexOf(id) > -1)) {
            ...
            let spliceIndex = this.personAddList_.findIndex(
              (it) => it.id.indexOf(id) > -1
            );
    ...
    checkAll(item, islabel = false) {
      ...
      if (item.attrs.childrenCount > 0 && !item.children.length > 0) {
        ...
        dsf.http
          .post(
            getDepts.action +
              `?doType=${getDepts.subTabKey}&asyncRequest=true&customRoot=${customRoot}&nodeType=${item.type}`,
            getDepts.urlParams || {}
          )
          .then(function (result) {
            ...
            getChild.forEach((inx) => {
              ...
              obj.children.push(inx);
            });
            item = obj;
            if (item.isopen) {
              if (
                self.extra.filterSelf &&
    ...
    selectAllUser(item,islabel){ //全选下级
            const self = this;
            if (item.attrs.childrenCount > 0 && item.children.length <= 0) {
                this.UserAll = []
                self.loopUser([item])
                let nowData = this.UserAll
                let getDepts = Object.assign({},this.currentTab);
                getDepts.action = getDepts.action.replace('async=true','async=false')
                console.log(getDepts);
                let customRoot =
                    item.id.split("_").length > 1 ? item.id.split("_")[item.id.split("_").length - 1] : item.id;
                dsf.http
                    .get(
                        getDepts.action +
                        `&doType=${getDepts.subTabKey}&asyncRequest=true&customRoot=${customRoot}&nodeType=${item.type}`, getDepts.urlParams || {}
                    )
                    .then(function (result) {
                        console.log(result);
                        if (result.status == '200') {
                            self.syncData(result.data.data.departmentList)
                            self.UserAll = []
                            self.loopUser(result.data.data.departmentList)
                            let data =data = self.UserAll
                            if (data.length != nowData.length){
                                result.data.data.departmentList.forEach(ele=>{
                                    if(ele.attrs.noselect){
                                        item.children.forEach(it=>{
                                            if (it.id.indexOf(ele.id) == '0') {
                                                it.children = ele.children
                                            }
                                        })
                                    }
                                })
    ...
    loopUser(data){  //递归获取全部下级人员
        data.forEach(ele=>{
            if (!ele.attrs.noselect && ele.attrs.isDirectory != 1) {
    ...
    userClick(user, islabel = false) {
      const self = this;
      // 节点选中时触发事件，如果监听事件中需异步改变checked，只能将loadChildData作为回调在改变后再次执行
      this.$bus.emit("userClickBus", {
        user,
        metaId: this.metaId,
        callback: self.loadChildData,
      });
      ...
      if (this.multiple) {
        if (!islabel) {
          ...
          if (user.checked) {
            ...
            let delIndex = this.personAddList_.findIndex(
              (item) =>
                user.id.split("_")[user.id.split("_").length - 1] == item.id
            );
            if (delIndex > -1) this.personAddList_.splice(delIndex, 1);
          }
        } else {
          // 仅操作下级
          if (user.children && user.children.length > 0) { // 全选下级
              let condition = []
              user.children.forEach(item =>{
                  (!item.attrs.noselect && item.attrs.isDirectory != 1) ? condition.push(item) : ''
              })
              if (condition.every(item =>!!item.checked == false)) {
                            if (this.checkTotal) {  //选中全部下级
                                this.UserAll = []
                                this.loopUser(user.children)
                                user.children.forEach((item) => {
                                    if(!!item.attrs.noselect || item.attrs.isDirectory == 1) return
