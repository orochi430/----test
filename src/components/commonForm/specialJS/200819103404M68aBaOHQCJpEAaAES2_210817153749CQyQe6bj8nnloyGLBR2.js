/*
 * @Author: jiangfeng
 * @Date: 2023-04-30 22:14:10
 * @Last Modified by: mikey.zhaopeng
 * @Last Modified time: 2023-09-21 13:58:45
 */
//会议变更
...
exportObj.initHandle = function(formData, formStruct, vueData) {
    if (!vueData.formData.main['opinionView'].value && formStruct.parameters.nodeId != '2o3sbXeCYmEMkA8') {
    ...
    vueData.$bus.on("blurBus", function(busObj) {
        ...
        dsf.http
            .post("fn/meetingRoom/getHysSqzyLimitAmount", {
                roomId: formData.main["c-meetingsqbg-004"].value,
                zyIds: formData.multinfos[busObj.subFormId].rows[busObj.subFormIndex][
            ...
            .then((res) => {
                ...
                if (res.status == "200" && res.data && res.data.data) {
                    console.log(res.data.data["230429145729ByUxYah864IgNNBRd1u"]);
                    if (
                        Number(busObj.value) >
                        Number(res.data.data["230429145729ByUxYah864IgNNBRd1u"])
                    ) {
                        dsf.layer.toast(
                            `该资源最大数量为${res.data.data["230429145729ByUxYah864IgNNBRd1u"]},请重新填写`
                        );
                        vueData.$set(
                            formData.multinfos[busObj.subFormId].rows[busObj.subFormIndex],
                            "C-MEETING-RESOURCE-005", { value: "" }
                        );
                    }
                }
            });
