...
const exportObj = {};
let listId = dsf.config.homePage.asset.form.entry.listId;
let cachedMetas = dsf.config.homePage.asset.form.cachedMetas;
...
exportObj.initHandle = function (formData, formStruct, vueData) {
  ...
  if (code) {
    vueData.$set(formData.main, "B0025", { value: code });
    // 从缓存里获取上一次保存的资产数据
...
exportObj.saveToNext = function (formData, formStruct, vueData) {
  // 保存资产数据缓存
  let assetInfoTemp = {};
  if (cachedMetas) cachedMetas.forEach(meta => {
    assetInfoTemp[meta] = formData.main[meta]
  });
...
// 扫码
let scan = (vueData, sendButtonParams, formData) => {
