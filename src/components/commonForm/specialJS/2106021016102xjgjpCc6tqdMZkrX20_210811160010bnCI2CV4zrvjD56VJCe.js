...
exportObj.initHandle = function (formData, formStruct, vueData){
    ...
    if (!formData.main['C-DB-0016']?.value) {
    ...
    vueData.$bus.off("subFormTreeSelect").on("subFormTreeSelect", function(busObj) {
        ...
        dsf.http.post('/ctrl/superviseV6/approval/getUserDept', {
            userDept: JSON.stringify(busObj.selectArr.map(item => {
                return {
                    userId: item.id,
                    deptId: item.pid,
                    userName: item.name
            ...
            if (res.data.code == 200) {
                res.data.data.forEach(async (item, index) => {
                    // 获取主办/协办已选人员
                    let comparisonArr = busObj.subFormId == '220808170715vHySOCUEffqz9jfTocG' ? formData.multinfos['2208081707395f0kQgjx9XflsmUXkxT'].rows : formData.multinfos['220808170715vHySOCUEffqz9jfTocG'].rows
                    // 重复则不添加
