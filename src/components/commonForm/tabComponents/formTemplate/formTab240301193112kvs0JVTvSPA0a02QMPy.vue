<template>
  <van-form validate-trigger="onChange" class="commonForm" ref="commonForm" v-if="formData.main != undefined && formMeta.controls != undefined">
    <div class="formLayout formTab240301193112kvs0JVTvSPA0a02QMPy" ref="form" :style="{ margin: isMobileForm ? dsf.config.commonForm.formPadding || '' : '0.32rem', borderTop: isMobileForm ? '' : '1px solid #333' }">
      <van-row type="flex" class="box" v-for="(row, rowindex) in controlRows" :key="row.rowindex">
        <template v-for="(col, index) in row">
          <van-col :span="rowFlex(row,index)" :id="`row${rowindex + 1}_col${index + 1}`" :style="{ backgroundColor: !isMobileForm ? '#fff' : '' }" :class="{ padLeftRight: !isMobileForm }">
            <div v-for="item in col" v-if="item" :key="item.controlId" style="padding: 0" :style="{ height: col.length == 1 ? '100%' : '' }" :class="classDivider(item, rowindex)" :id="`ctr_${item.controlId}`">
              <transition name="fade">
                <form-layout v-show="item.show && item.propertyShow" :isMobileForm="isMobileForm" :rowCteNum="row.length" :allFile="allFile" :ref="item.metaId" @controlEditOpinion="controlEditOpinion" @controlShareOpinion="controlShareOpinion" :parameters="formMeta.parameters" :controlData="getData(item)" :metaData="item" :formData="formData" :formMeta="formMeta" :topformData="formData" :topformMeta="formMeta" :relationList="relationList" :form-params="formParams"></form-layout>
              </transition>
            </div>
          </van-col>
        </template>
  ...
  methods: {
    handleControlEditOpinion() {
      ...
      if (ctr) {
        ...
        $(".formTab240301193112kvs0JVTvSPA0a02QMPy").css("margin-bottom", '3.2rem')
        // }
      }
    },
    handleHeaderLabel() {
      if (this.controlRows[0] && this.controlRows[0][0] && this.controlRows[0][0][0]?.controlType == "LabelForTextControl") {
        ...
        $(".formTab240301193112kvs0JVTvSPA0a02QMPy").addClass("noborder")
      }
      ...
      $(".noDivider").addClass("padLeftRight")
      $(".controlOpinionView").parent().parent().addClass("bac")
      $(".controlOpinionView").parent().parent().parent().parent().parent().removeClass("padDivider")
      $(".controlOpinionView").parent().parent().parent().parent().parent().parent().removeClass("padLeftRight")
      $(".control_mobile_fl").parent().parent().parent().parent().removeClass("padDivider")
      $(".control_mobile_fl").parent().parent().parent().parent().parent().removeClass("padLeftRight")
      $(".control_mobile_fl").parent().addClass("bac")
...
<style lang="scss">
.formTab240301193112kvs0JVTvSPA0a02QMPy {
  &.noborder {
...
.formTab240301193112kvs0JVTvSPA0a02QMPy > .box {
  .noAfter::after {
    border: 0 !important;
  }
  font-family: KT_GB2312 !important;
  .pad_layout_box > .van-col:last-child {
    padding: 0.05rem 0.15rem;
}
 .controlTextarea .van-field .van-field__control{
  ...
  .controlOpinionView .opinionBox:not(:first-child)::before {
    position: absolute;
    box-sizing: border-box;
    content: " ";
    pointer-events: none;
