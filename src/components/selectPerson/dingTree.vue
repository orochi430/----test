...
<div style="width:100%;height:100%;" class="dsf-tree">
    <div v-for="(item,index) in personSelectData" :key="'index1'+index" style='overflow-y: hidden;'>
        <div class="flex-box tree_line">
            ...
            <div class="tree_tit" @click="checkAll(item)"><span :class="{tit_width:!item.children.length>0}">{{item.showName}}</span><span v-if="item.children.length>0">({{item.children.length}})</span></div>
            <div class="isShowChiled" @click="checkAll(item)" v-if="item.children.length>0">
                <van-icon v-if="item.isopen" name="arrow-up" color="#3296fa"/>
                <van-icon v-else name="arrow-down" color="#3296fa"/>
