...
<div class="inspector-condition">
    ...
    <!-- </div> -->
    <div class="_header">
        <div :class="isActive == 'left' ? '_active' : ''" class="_btn" @click="clickHeader('left')">未反馈<span v-if="noFeedbackTotal">({{noFeedbackTotal}})</span></div>
        <div :class="isActive == 'right' ? '_active' : ''" class="_btn" @click="clickHeader('right')">已反馈<span v-if="alreadyFeedbackTotal">({{alreadyFeedbackTotal}})</span></div>
    </div>
    <!-- 未反馈 -->
    <div v-if="isActive == 'left'" :class="{'_content':true,'_weifankui':isConditionActive=='fkqs2','_yifanhui':isConditionActive!='fkqs2'}">
        <ul>
            <li v-for="(item, index) in wfkData" :key="index" class="_cell">
                <span class="icon iconfont icontouxiang"></span>
                <span class="_cell__title">{{ item.cbrText }}</span>
                <span :class="item.status == 1 ? '_yi _cell__value' : '_wei _cell__value'">{{ item.cbbmText }}</span>
            </li>
        </ul>
        <div class="_footer">
            <!-- <Ding-btn v-if="isConditionActive == 'fkqs2'" :prarms="dingObj" :options="{ size: 'large', type: 'info' }" text="DING一下"></Ding-btn> -->
            <Ding-btn :pk="params.pk" :prarms="dingObj" :options="{ size: 'large', type: 'info' }" text="DING一下"></Ding-btn>
        </div>
    </div>
    <!-- 已反馈 -->
    <div v-if="isActive == 'right'" class="_content _yifanhui">
        <ul>
            <van-collapse v-model="activeNames">
                <van-collapse-item class="vanCollapse" :value="'('+item.data.length+'次)'" :name="index" v-for="(item, index) in listData" :key="index">
                    <template #title>
                        <div style="font-size: var(--font_size_2);display:flex;"><span class="icon iconfont icontouxiang" style="color:#3296fa;margin-right:10px;font-size: var(--font_size_0);"></span><span>{{item.deptName}}</span> </div>
                    </template>
                    <li class="" :class="{cell_li:isActive == 'right'}" @click="handleClick(itm)" v-for="(itm, inx) in item.data" :key="inx">
                        <div class="cell_line">
                            <div class="cell_name">
                                <span class="_cell__title">{{ itm.feedBackUserName }}</span>
                            </div>
                            <div class="cell_time">
                                <span class="_yi _cell__value">{{ itm.strDate }}</span>
                            </div>
                        </div>

                        <div class="cell_content ellipsis">反馈情况：{{ itm.content }}</div>
                    </li>
                </van-collapse-item>
            </van-collapse>
        </ul>
...
export default {
    ...
    methods: {
        initList() {
            ...
            dsf.http
                .get(`fn/superviseV5/feedback/getFeedBackByFid?id=${id}&rank=2`, self.params)
                .then(function (res) {
                    let {
                        data
                    } = res;
            ...
            dsf.http
                .get(`fn/superviseV5/feedback/getNoFeedBackByFid?id=${id}&rank=2`, self.params)
                .then(function (res) {
                    let {
                        data
                    } = res;
        ...
        // 数据过滤
        filtersData() {
            if (this.isConditionActive == "fkqs0") { //往期
            ...
            } else if (this.isConditionActive == "fkqs1") { //上期
                this.noFeedbackTotal = this.listDataMock.lenght > 1 ? this.listDataMock[1].fkzt1.length : 0
                this.alreadyFeedbackTotal = this.listDataMock.lenght > 1 ? this.listDataMock[1].fkzt2.length : 0
            } else { //本期
            ...
            if (this.isConditionActive == "fkqs1" && this.isActive == "left") {
                // 上期 未反馈
                this.listData = this.listDataMock.lenght > 1 ? this.filterData(this.listDataMock[1].fkzt1) : [];
            }
            if (this.isConditionActive == "fkqs1" && this.isActive == "right") {
                // 上期 已反馈
                this.listData = this.listDataMock.lenght > 1 ? this.listDataMock[1].fkzt2 : [];
            }
            ...
            this.listData.forEach(item => {
                ...
                if (item.sjfksj) {
                    ...
                    item.time = times[2] + "-" + times[3] + " " + times[5] + ":" + times[6]
                }
                if (users.indexOf(item.foreignId) < 0) {
                    users.push(item.foreignId);
                }
        ...
        handleClick(item) {
            this.$router.push(`/commonForm/${item.moduleId}/${item.feedBackId}`)
            // this.$router.push({
            //   name: "commonForm/",
            //   params: {
            //     pk: item.id,
