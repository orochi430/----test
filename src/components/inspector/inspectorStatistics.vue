<template>
	<div id="inspector-statistics">
		...
		<div class="select-container">
			...
			<div>
				<span @click="openStartTimePicker">{{dateFormat(startDate, 'yyyy-MM-dd')}}</span>
				<span>至</span>
				<span @click="openEndTimePicker">{{dateFormat(endDate, 'yyyy-MM-dd')}}</span>
			</div>
		</div>
		<div class="chart-container">
			<v-chart :options="polar" class="" />
		</div>
		...
		<van-popup v-model="isShowStartTimePicker" position="bottom">
			<van-datetime-picker v-model="tempStartDate" type="date" title="选择起始时间" :min-date="minDate" :max-date="maxDate" @cancel="isShowStartTimePicker=false"
			 @confirm="startDateConfirm" />
		</van-popup>
		<van-popup v-model="isShowEndTimePicker" position="bottom">
			<van-datetime-picker v-model="tempEndDate" type="date" title="选择结束时间" :min-date="minDate" :max-date="maxDate" @cancel="isShowEndTimePicker=false"
	...
	export default {
		...
		data() {
			return {
				...
				startDate: new Date(new Date().getFullYear(), 0, 1), //设置默认开始时间为当前年1月1日
				endDate: new Date(), //设置默认结束时间为当前天
				tempStartDate: new Date(new Date().getFullYear(), 0, 1), //时间选择器临时数据保存
						...
						itemStyle: {
							normal: {
								label: {
									...
									position: 'inner',
									formatter(data) {
										return data.value + '件' + data.percent.toFixed(0) + '%';
									},
									textStyle: {
										color: '#000',
										fontSize: 15,
