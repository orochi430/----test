    ...
    <div v-if="isActive == 'right'" class="_content _yifanhui">
      <ul>
        <van-collapse v-model="activeNames">
          <van-collapse-item
            class="vanCollapse"
            :value="'(' + item.data.length + '次)'"
            :name="index"
            v-for="(item, index) in listData"
            :key="index"
          >
            <template #title>
              <div style="font-size: var(--font_size_2); display: flex">
                <span
                  class="icon iconfont icontouxiang"
                  style="color: #3296fa; margin-right: 10px; font-size: var(--font_size_0)"
                ></span
            ...
            >
              ...
              <div class="cell_content ellipsis">
                反馈情况：{{ itm.content }}
              </div>
            </li>
          </van-collapse-item>
...
export default {
  ...
  methods: {
    ...
    handleClick(item) {
      this.$router.push(`/commonForm/210608105939mkuTUSzHdhowHPHxlY8/2109121906368wtdwi06olY4uTNhozf`);
      // this.$router.push({
      //   name: "commonForm/",
      //   params: {
      //     pk: item.id,
