  ...
  >
    ...
    <div class="card">
      <div class="cell flex jb">
        <img
          src="static/images/peopleCongress/wdsfp-icon-OrganizationalUnit.png"
          alt=""
        />
        <span>单位部门：</span
        ><span style="flex:1">{{ meetingInfo.dept }}</span>
      </div>
      <div class="cell flex jb">
        <img
        ...
        />
        <span>联系电话：</span
        ><span style="flex:1;line-height:0.4rem">{{ meetingInfo.mobile }}</span>
      </div>
      <div class="code">
...
export default {
  ...
  methods: {
    ...
    getMyQrCode() {
      ...
      dsf.http
        .post(`fn/peopleCongress/myQrCode`, { masterId: self.meetingInfo.id })
        .done(res => {
          if (res.type == "success") {
            self.code = `${res.data}&ran=${Math.random()}`;
          }
        })
        .error(err => {
          console.log("myQrCode-err", err);
        })
        .always(() => {
    ...
    queryStatus() {
      ...
      if (self.$route.name == "peopleCongressQrCode") {
        dsf.http
          ...
          .error(err => {
            console.log("codeScanningStatus-err", err);
          })
          .always(() => {});
      }
    },
    ...
    openMySocket() {
      ...
      self.oaScoket.onScanCodeAfter(
        store.loadSessionStore("user").userId,
        result => {
