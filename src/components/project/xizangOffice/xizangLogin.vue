<template>
    <div class="box" :style="bgImgStyle">
        <div class="banner" :style="logImgStyle" @click="showDeviceId">
        ...
        <div v-show="loginType == 'user'" class="formbox loginForm">
            ...
            <van-form @submit="login">
                ...
                <div class="btnbox" style="margin-top: 30px">
                    ...
                    <van-button v-else style="width: 100%; font-size: 19px" round type="info" native-type="submit">登录
                    </van-button>
                </div>
            ...
            <van-icon v-if="dsf.config.login.isShowIcon" name="static/images/icon-img.png" size="50"
                class="login_icon" />
        </div>
        ...
        <div v-if="loginType == 'sms'" class="formbox loginForm">
            ...
            <van-form ref="smslogin" @submit="login">
                ...
                <div class="sendCode">
                    ...
                    <van-button :text="smsBtnText" :disabled="smsBtnEnable" size="small" type="primary"
                        native-type="button" @click="sendSmsCode()"></van-button>
                </div>
                ...
                <div class="btnbox" style="margin-top: 30px">
                    <van-button style="width: 100%; font-size: 19px" round type="info" native-type="submit">登录
                    </van-button>
                </div>
                <div class="changeType">
        ...
        <div v-if="loginType == 'gesture'" class="formbox loginForm">
            <gesture gestureType="check" @gestureError="forgetGesture"></gesture>
            <div class="changeType">
                <span @click="changeLoginType('user')">账号登录</span>
                <span v-if="isShowSmsLogin > 0" @click="changeLoginType('sms')">短信验证码登录</span>
            </div>
...
export default {
    ...
    data() {
        return {
            ...
            bgImgStyle: {
                background: "url('static/images/xizangOffice/loginbg.png') no-repeat",
    ...
    beforeRouteLeave(to, from, next) {
        ...
        if (!tocken && from.name == "xzLogin" && to.name != "xzLogin") {
            dsf.layer.toast(
                "您进行了未授权的访问，请重新登录！",
                undefined,
                function () {
    ...
    methods: {
        ...
        login() {
            let self = this
            if (store.getClientName() == "app") {
                if (dsf.config.login.checkCA) {
                    dsf.http.post(`fn/caAttestController/getCAActivationCode?logName=${self.logname}`).then(function (data) {
                        console.log(data)
                        if (data.data.code == 200) {
                            function checkCA() {
                                $BJCA.getCerStatus({ messId: data.data.data.msspId }, function (cerData) {
                                    console.log(cerData);
                                    /*0x14300001：用户在本地无证书；
                                    89005003：用户信息不存在；
                                    89005004：用户已被注销，请联系管理员；
                                    89005005：用户已被冻结，请联系管理员；
                                    89005006：用户设备信息有误，请重新下载设备证书；
                                    89005007：用户设备已被注销，请重新下载设备证书；
                                    89005008：用户设备尚未激活，请重新下载设备证书；
                                    89004041：用户证书过期超过一个月，请重新下载设备证书；
                                    89004042：用户证书过期未超过一个月，请重新下载设备证书*/
                                    switch (cerData.userStateCode) {
                                        case "200":
                                            self.login2();
                                            break;
                                        case "89005003":
                                            dsf.layer.toast("用户信息不存在");
                                            break;
                                        case "89005004":
                                            dsf.layer.toast("用户已被注销，请联系管理员");
                                            break;
                                        case "89005005":
                                            dsf.layer.toast("用户已被冻结，请联系管理员");
                                            break;
                                        case "0x14300001":
                                        case "89005006":
                                        case "89005007":
                                        case "89004041":
                                        case "89004042":
                                            $BJCA.getCertWithPinAuth({ authCode: data.data.data.code, pin: "111111" }, function (authData) {
                                                console.log(authData);
                                                if (authData.errCode == "0x00000000") {
                                                    self.login2();
                                                } else {
                                                    dsf.layer.toast(authData.errMsg);
                                                }
                                            }, function (error) {
                                                alert(error);
                                            });
                            ...
                            if (typeof $BJCA == 'undefined') {
                                document.addEventListener("deviceready", function () {
                                    console.log("deviceready_autologin");
                                    checkCA();
                                });
                            } else {
                                checkCA();
        ...
        login2() {
            ...
            if (self.loginType == "user") {
                ...
                url = `/fn/mobileSmsAuth/login?mobile=${self.phone}&code=${self.phonecode}`;
            }
        ...
        sendSmsCode() {
            ...
            self.$refs.smslogin.validate("phone").then(() => {
                ...
                return dsf.http.get(`/fn/mobileSmsAuth/sendSms?mobile=${self.phone}`, {})
            }).then((data) => {
                console.log(data);
