...
<div class="warpbox">
    ...
    <div class="totals">
        ...
        <div @click="toList(totalData.all_nuclein_num,{nuclein_Status: '-1', nuclein_Overdue: '-99', vaccin_Result: '-99', redCode: '-99', yellowCode: '-99'})">
            <span>核酸阳性</span>
            <span style="color: #F66C6C;">{{totalData.all_nuclein_num || 0}}</span>
        </div>
        <div @click="toList(totalData.all_nuclein_late_num, {nuclein_Status: '-99', nuclein_Overdue: '1', vaccin_Result: '-99', redCode: '-99', yellowCode: '-99'})">
            <span>核酸超期</span>
            <span style="color: #FA6400;">{{totalData.all_nuclein_late_num || 0}}</span>
        </div>
        <div @click="toList(totalData.all_vaccin_num, {nuclein_Status: '-99', nuclein_Overdue: '-99', vaccin_Result: '-1', redCode: '-99', yellowCode: '-99'})">
            <span>未按要求接种疫苗</span>
            <span style="color: #F7B500;">{{totalData.all_vaccin_num || 0}}</span>
        </div>
        <div style="border-bottom: none;" @click="toList(totalData.all_red_num, {nuclein_Status: '-99', nuclein_Overdue: '-99', vaccin_Result: '-99', redCode: '10', yellowCode: '-99'})" >
            <span>健康码（红码）</span>
            <span style="color: #E02020;">{{totalData.all_red_num || 0}}</span>
        </div>
        <div style="border-bottom: none;" @click="toList(totalData.all_yellow_num, {nuclein_Status: '-99', nuclein_Overdue: '-99', vaccin_Result: '-99', redCode: '-99', yellowCode: '01'})">
            <span>健康码（黄码）</span>
    ...
    methods: {
        toTotalList() {
            if(this.totalData.all_num) {
                this.$router.push({
                    path: `commonlist/220330142018zVPxmR3EO0HoyRoRS5O/220319194130l6pirMZJe74yeqvID8I`,
                })
            }
        },
        toList(num, obj) {
            if(num) {
                this.$router.push({
                    path: `commonlist/220330142037OJwEqORfSPc2O1T1aD5/220319194130l6pirMZJe74yeqvID8I`,
                    query: {
        ...
        /**
         *  id  所属父节点的ID
         *  type  1 社区  2 区  3 街道  4 总
         */
        async initData() {
            let loading = dsf.layer.loading()
            Promise.all([this.loadTotalData('', 4),
                this.getHistoryCharts('', 4),
                this.getChartsByArea('', 4)]
            ).then((result) => {
        ...
        getHistoryCharts(id, type) {
            ...
            return dsf.http.post("/ctrl/sqa/getHistoryCharts?id=" + id + "&type=" + type).then(function (data) {
                if (data && data.data.code === "200") {
                    ...
                    let dataSource = {
                        legend: ["核酸阳性", "未按要求接种疫苗", "核酸超期", "健康码异常"],
                        xAxis: [],
                        values1: [],
                        values2: [],
                        values3: [],
