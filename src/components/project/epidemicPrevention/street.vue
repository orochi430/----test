...
export default {
    ...
    methods: {
        ...
        async onStreetConfirm(value, index) {
            ...
            //先获取选择的社区下面的数据
            Promise.all([this.loadTotalData(value.code_value, 3),this.getHistoryCharts(value.code_value, 3),this.getChartsByArea(value.code_value, 3)]).then((result) => {
        ...
        initStreetData() {
            ...
            dsf.http.post("/ctrl/sqa/getAreaByUserId?userid="+user_id).then(async function (data) {
                if (data && data.data.code === "200") {
                    that.streets = data.data.data.town;
                    console.log(that.streets)
                    if(that.streets.length > 0) {
                        Promise.all([that.loadTotalData(that.streets[0].code_value, 3),
                            that.getHistoryCharts(that.streets[0].code_value, 3),
                            that.getChartsByArea(that.streets[0].code_value, 3)]
                        ).then((result) => {
                            dsf.layer.closeLoading(loading)
                        }).catch((error) => {
                            dsf.layer.toast(error)
            ...
            return dsf.http.post("/ctrl/sqa/getHistoryCharts?id=" + id + "&type=" + type).then(function (data) {
                if (data && data.data.code === "200") {
                    ...
                    rows.map((item, index) => {
                        ...
                        dataSource.values1.push(parseInt(item.nuclein_late_num))
                        dataSource.values2.push(parseInt(item.health_num))
                        dataSource.values3.push(parseInt(item.vaccin_num))
                    })
