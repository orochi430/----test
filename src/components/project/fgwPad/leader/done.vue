        ...
        <van-list :finished="finished" :finished-text="finishedTips" v-model="loading" @load="onLoad">
            <debounce-click v-for="(item, index) in list" :key="index">
                <div :class="['flex', 'ac', 'list-item']" @click="goForm(item)">
                    <van-checkbox v-model="item.checked" shape="square" icon-size="16px" v-if="showCheckAll"></van-checkbox>
                    <!-- <img
              src="static/images/project/fgwPad/fw.png"
              alt=""
              @click="goForm(item)"
            /> -->
                    <span class="icon iconfont iconfawen" style="font-size: 38px; margin: 0px 10px; color:#087FE3;"></span>

                    <div class="main">
                        <div class="ellipsis title" :class="{ bold: !item.isRead }">
                            <img v-if="showJinJi(item)" :src="showJinJi(item)" class="jinji" />
                            <!-- <span>[{{ item["MAININFO.A0005"] }}]</span> -->
                            <span>{{ item["MAININFO.B0001"] }}</span>
                        </div>
                        <div class="flex jb sub-title">
                            ...
                            <span style="flex: 1" v-if="item['MAININFO.A0022']">
...
export default {
    ...
    methods: {
        ...
        getList() {
            ...
            dsf.http
                .get(
                    "/fn/list/20012015444389JJL1Af0XRDv0xWYmy/mobile/q?moduleId=190620203722hdlBjofE3g9kzT1h4ct",
                    params
                )
                .then((res) => {
        ...
        goForm(item) {
            this.$router.push({
                name: "commonForm",
                params: {
                    moduleId: item["MAININFO.A0004"] || item["MAININFO.A0004.value"],
                    pk: item["MAININFO.A0001"],
                },
            });
        },
    },
