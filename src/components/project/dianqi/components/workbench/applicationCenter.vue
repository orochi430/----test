  ...
  <div>
    ...
    <template v-if="isShowTab">
      <my-application-card v-if="isShowMyMenus || justShowMyMenus" :menu="myMenus" :showIcon="showIcon"
        :isHideTitle="isHideTitle" :title="myMenusName" :rowNum="rowNum"
        :class="{ 'title-outside': outsideTitle }"></my-application-card>
      <van-tabs :line-width="active.length + 'em'" v-if="!justShowMyMenus" v-model="active" scrollspy sticky
        :offset-top="offsetTop" :class="['menu-tab', { 'theme-tab': positionTab.useTheme }]" ref="tabs" @scroll="change">
    ...
    <div v-else>
      <my-application-card v-if="justShowMyMenus" :menu="myMenus" :showIcon="showIcon" :isHideTitle="isHideTitle"
        :title="myMenusName" :rowNum="rowNum" :class="{ 'title-outside': outsideTitle }"></my-application-card>
...
export default {
  ...
  activated() {
    if (this.isSetUserShortcutDing && false) {
      this.myMenus = this.storeMyMenu
    } else {
      // this.myMenus = dsf.util.loadLocalStore('mySettingDQ')
      this.getMenu()
    }
  ...
  methods: {
    ...
    getMyMenus() {
      return new Promise((resolve, reject) => {
        ...
        //   }
        // }
        dsf.http.get("fn/application/portal/getUserCustomApp").done(res => {
          // let mySettingMenu = []
          // this.forFunc(res.data.data, mySettingMenu)
          if (res.type == "success") {
            this.myMenus = res.data.map(item => ({
              id: item.id,
              name: item.appName,
              url: item.mobileUrl,
              isshow: true,
          ...
          dsf.util.saveToLocalStore('mySettingDQ', this.myMenus)
          resolve()
        })
      })
    },
