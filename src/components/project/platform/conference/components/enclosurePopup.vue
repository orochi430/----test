...
export default {
  ...
  data() {
    return {
      ...
      currentFile: {},
      fileUrl: '',
      isSignature: dsf.util.loadSessionStore("meeting").allowComments != undefined ? dsf.util.loadSessionStore("meeting").allowComments : true ,
  ...
  methods: {
    open(file) {
      if (dsf.util.isImage(file.fileName)) {
          let url = ''
          if (this.$route.name == "peopleCongressFile" || file.officeModuleId === '210902111649AhoKqvPNPCT90ktfWET') { // 文件查询
              url = dsf.url.getWebPath(`fn/conferenceFile/download?pk=${file.fileId}`);
          } else { // 消息通知
              url = dsf.url.getWebPath(`fn/mobileFileDownload/download?fileId=${file.fileId}&moduleId=${file.moduleId}&fileName=${file.fileName}`);
          }
          console.log(url)
        ...
        if (!dsf.config.commonForm.LocalDocPreview) { // pdf.js打开
          let previewUrl =dsf.url.getServerUrl(`fn/office/openMobileOffice?fileId=${file.fileId}&moduleId=${file.moduleId}&nodeId=${file.nodeId}&flowId=${file.flowId}&filename=${file.fileName}&x-auth-token=${dsf.util.loadSessionStore("loginToken")}&handwriting=1&screenWidth=${document.body.clientWidth}`) ;
