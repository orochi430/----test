      ...
      <van-form @submit="login">
        <van-field v-model="logname" :rules="[{ required: true }]" name="账号" placeholder="请输入用户名" center>
          <template #left-icon>
            ...
            <span class="icontext">&nbsp账号&nbsp&nbsp&nbsp</span>
          </template>
        </van-field>
        ...
        <van-field v-model="txtpassword" :rules="[{ required: true }]" type="password" name="密码" placeholder="请输入密码" center>
          <template #left-icon>
            ...
            <span class="icontext">&nbsp密码&nbsp&nbsp&nbsp</span>
          </template>
        </van-field>
        <!-- 湖北增加动态临牌登录框 -->
        ...
        <!-- <van-row type="flex" justify="space-between" class="auto" >
                <van-checkbox  v-model="checked" icon-size="16" shape="square">
                    自动登录
                </van-checkbox>
        ...
        <div class="btnbox" style="margin-top: 30px">
          ...
          <van-button v-else style="width: 100%; font-size: 19px,height: 52px;" round type="info" native-type="submit">登&nbsp&nbsp录</van-button>
        </div>
  ...
  methods: {
    ...
    async login() {
      ...
      if (autoToken && dsf.config.login.tokenAutologin && this.checked) { // 使用authtoken自动登录
        ...
        //记录上次登录类型
        lastLoginType = "user"
        // 项目组特殊登录 配置specialLoginParams模板字符串取参后转入posyData对象
