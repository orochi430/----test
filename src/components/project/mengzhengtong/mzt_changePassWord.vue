<template>
  <div id="mzt_changePassWord">
    <div class="content">
      <!-- 表单盒子 -->
      <van-form ref="editPassword_form" @submit="onSubmit">
        <!-- 旧密码 -->
        <van-field
          v-model="editPassword_form.oldPsw"
          clearable
          type="password"
          name="oldPsw"
          label="旧密码"
        ...
        <!-- 新密码 -->
        <van-field
          v-model="editPassword_form.newPsw"
          clearable
          type="password"
          name="newPsw"
          label="新密码"
        ...
        <!-- 确认密码 -->
        <van-field
          v-model="editPassword_form.checkPsw"
          clearable
          type="password"
          name="checkPsw"
          label="确认密码"
  ...
  computed: {
    minPwd() {
      ...
      return dsf.config.setting.hasOwnProperty("SYS-SEC-0001")
        ? dsf.config.setting["SYS-SEC-0001"] || 6
        : 6;
    },
    strengthPwd() {
      // 密码强度
      return dsf.config.setting.hasOwnProperty("SYS-SEC-0002")
        ? dsf.config.setting["SYS-SEC-0002"] || 1
        : 1;
    },
    point() {
            ...
            dsf.http
              ...
              .then((res) => {
                ...
                if (res.data.type === "success") {
                    console.log("格尔的密码修改成功");
                } else {
                  console.log("格尔的密码修改失败");
                }
              })
              .catch((err) => {
                console.log(err);
              });
            if (this.isIm && window.native && window.native.isNative) {
              //判断是不是
              ...
              this.dsf.layer.toast("修改密码成功,请重新登录", "success", () => {
                if (
                  dsf.util.loadSessionStore("homePage") == "/mzt_changePassWord"
                )
                  dsf.util.removeSessionStore("homePage");
                dsf.util.logOut();
              });
            }
          } else {
