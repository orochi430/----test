...
export default {
    ...
    methods: {
        ...
        getUser() {
            let self = this;
            let user = dsf.util.loadLocalStore("user") || dsf.util.loadSessionStore("user");
            if (dsf.config.staticJson.enable) { // 静态数据演示环境
                // self.logname = 'tianxin';
                self.name = '李敬梭';
                self.mobile = '15335312882';
                self.login();
            } else if (self.name && self.mobile) { // url传参登录
                // } else if (self.$route.query.name && self.$route.query.mobile) { // url传参登录
                // self.name = self.$route.query.name;
                // self.mobile = self.$route.query.mobile;
                self.login();
            } else if (user && user.name && user.mobile) { // 赣政通
                self.name = user.name;
                    ...
                    if (self.renda && self.renda.getUserByToken) {
                        ...
                        self.renda.getUserByToken({
                            success: (res) => {
                                ...
                                // {"certNo":"42028119******0018","certType":"IDENTITY_CARD","deputy":false,"nbf":1644****62,"phone":"153****9312","certName":"肖承尧","exp":1644****62,"userId":10,"iat":1644****62}
                                // 成功的回调（返回用户信息）
                            ...
                            fail: (err) => {
                                ...
                                dsf.layer.toast(`${err.msg}`, false, () => self.navigateToApp());
                            }
                        })
                    }
                });
        ...
        login() {
            ...
            let token = dsf.md5(`人大会务${self.mobile}${self.name}${self.cardId}系统`);
                ...
                dsf.http
                    ...
                    .done(result => {
                        ...
                        if (result && result.code == 200) {
                            ...
                            if (self.toUrl) { // 登录后跳转指定页面
                                self.toUrl = decodeURIComponent(self.toUrl);
                            } else if (result.data.meetingType && result.data.meetingType.length == 1) { // 仅1个会议类型时跳过会卡进入首页
