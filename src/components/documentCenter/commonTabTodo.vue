<template>
	<div :class="['ds-hometodocenter', hasBottom ? 'has-bottom' : '']">
		<van-pull-refresh v-model="isLoading" success-text="刷新成功" @refresh="onRefresh">
			<van-list :finished="finished" :finished-text="data.length ? finishedTips : ''" v-model="loading" :offset="500"
			 :immediate-check="false" @load="onLoad">
				<DsList :tabObj="params" :type="tab" :data="data" :columns="columns" @mainClick="handleActionClick" @commonClick="handleActionClick"
				 formMethod="info">
		...
		<div v-if="hasBottom" class="footer">
			<DsfButtonToolbox :more="true" :buttons="listButtons" @mainClick="handleActionClick" @commonClick="handleActionClick" />
		</div>
		<van-popup :get-container="getContainer" :style="{ height: '90vh' }" v-model="showPerson" :close-on-click-overlay="false"
		 lock-scroll position="bottom" round>
			<selectPerson :extra="extra" :type="0" @getSelectUser="getSelectUser" @cancePerson="cancePerson"></selectPerson>
		</van-popup>
		<van-popup :get-container="getContainer" v-model="showFlowSend" :close-on-click-overlay="false" lock-scroll style="height:90vh"
		...
		<van-popup :get-container="getContainer" v-model="showFlowBack" :close-on-click-overlay="false" lock-scroll style="height:90vh"
		 position="bottom" round>
	...
	export default {
		...
		methods: {
			...
			async getBadge() {
				...
				let arrPromise = []
				let url = this.getReplaceParams(data.url);
				dsf.http.post(url, data.countParams).then(res => {
