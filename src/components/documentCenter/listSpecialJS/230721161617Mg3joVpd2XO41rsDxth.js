...
exportObj.link = function (listid, moduleId, pk, vueData, itemValue) {
  ...
  let url = ''
  if (itemValue["C-R-RCV-0012.value"] == 1) {  //是否有反馈
    dsf.http.get("ctrl/meetingRemote/getSingleMeetingNoticeBySw",{
        "pk": itemValue["A0003"]
    }).then((res) => {
        console.log(res);
    ...
    if (itemValue["A0003"]) {
      dsf.http.get("ctrl/remoteExchangeData/getInboxData",{
          "pk": itemValue["A0003"]
      }).then((res) => {
          console.log(res);
        if (res.data.code == 200) {
          let data = res.data.data
          if (data.pid&&data.pnid) {  //本人有待办
              url = `/commonForm/${data.moduleId}/${itemValue["A0003"]}?pk=${itemValue["A0003"]}&todoId=${data.pid + '_' + data.pnid}`
          }else{
            if (data.status==1) {   //其他人有代办
