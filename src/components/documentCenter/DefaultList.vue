<template>
  <!--  -->
  <van-loading v-if="!alreadyLoaded" size="24px" style="text-align: center;">加载中...</van-loading>
  <div class="ds-signinlist" :class="{ scrollBox: showCheck }" ref="container" v-else>

    <van-sticky :container="stickyContainer" :offset-top="oppsetTop ? dsf.util.getOffsetTop(oppsetTop) : dsf.util.getOffsetTop()"
    ...
    <van-pull-refresh v-else v-model="isLoading" success-text="刷新成功" @refresh="onRefresh">
      <van-list :finished="finished" :finished-text="finishedTips" v-model="loading" @load="onLoad" offset="100">
        <DsList ref="dslist" :data="data" :spHandleJs="spHandleJS" :listId="listId" :listmoduleId="moduleId"
          ...
          :showCheck="showCheck" :mobileStyle="mobileStyle" :keyColumn="keyColumn" @checkClick="checkClick"
          @mainClick="handleActionClick" @commonClick="handleActionClick" :showSubTitle="showSubTitle"
          :checkIds="checkIds" :link-to="linkTo" :showTips="showTips" @itemClick="currentDataClick"/>
      </van-list>
    </van-pull-refresh>
    <div class="utilBox disfr" v-if="showCheck">
      <van-checkbox v-if="checkType == 'checkBox'" v-model="checkAll" style="font-size: 0.28rem;padding-right: 10px" @click="changeCheckAll">全选</van-checkbox>
    ...
    <van-action-sheet v-model="show" class="searchSheet" position="bottom" round>
      ...
      <div class="sheetBox flex1">
        <component v-for="(item, index) in controlData" :key="index" :meta-data="item" :is="item.control" v-show="showAllSearch || index < 2">
        </component>
      </div>
    ...
    <dsf-fab v-if="!isHideListButton &&
      ((listButtons &&
        ...
        showAddForm) &&
      !showCheck && (!$route.query.isFooterBtnShow&&!dsf.config.commonList.isFooterBtnShow)" :fab-items="listButtons.mainButtons" title-key="name" @clickItem="btnAction">
    </dsf-fab>
    <div v-if="!isHideListButton &&
      ((listButtons &&
        ...
        listButtons.mainButtons.length) ||
        showAddForm) &&
      !showCheck && ($route.query.isFooterBtnShow||dsf.config.commonList.isFooterBtnShow)" style="height: 1.2rem" class="footer-list">
      <div class="footer">
    ...
    <van-popup :get-container="getContainer" ref="sendPop" v-model="showFlowSend" lock-scroll :style="{height: dsf.config.commonFlow.nodeDisplayType == 'showSimple' ? '100vh' : '90vh'}"
      position="bottom" :round="dsf.config.commonFlow.nodeDisplayType != 'showSimple'" @close="closeSendFilePopup">
      <sendFile v-show="showFlowSend" ref="sendFile" :form-data="selected" @change="handleFlowSendChange"
        @close="closeSendFile" @firstBatchSend="firstBatchSend"/>
    </van-popup>
...
import DsfListTable from "./DsfListTable";
...
import specialJS from "./listSpecialJS";
...
export default {
  ...
  activated() {
    ...
    if (this.$route.meta.keepAlive) {
      ...
      this.init();
      if (this.currentClickScrollTop != 0 && this.currentClickScrollTop != document.documentElement.scrollTop) {
        document.documentElement.scrollTop = this.currentClickScrollTop
      }
    }
  },
  methods: {
    ...
    currentDataClick(data) {
      console.log(data)
      this.currentClickPage = Math.ceil(data.index / this.limit) + 1
      this.currentClickScrollTop = data.scrollTop
    },
    goBack() {
    ...
    // 成都一体化 列表按钮跳转commonTabs
    trackingUpdates(item) {
        this.$router.push({
            path: `/commonTabs/${item.action}`,
        })

    },
    async fetchData(isFirst, isEventbus) {
      let url = this.listInterface;
      let params = {
        limit: isEventbus ? this.limit * this.currentClickPage : this.limit,
      ...
      dsf.http
        ...
        .then(({ status, data }) => {
            ...
            if (!this.isSearch && this.isFirst) {
              ...
              this.controlData.forEach(item => {
                if (item.defaultValue)
                  ...
                  if (this.listInferenceParams.hasOwnProperty(`query_${item.metaData}`)) {
                    item.value = this.listInferenceParams[`query_${item.metaData}`]
                  }
              });
            }
          }
          ...
          if (status === 200) {
            ...
            if (
              ...
              datas.length === 0 ||
              datas.length < (isEventbus ? this.limit * this.currentClickPage : this.limit)
            ) {
              // 加载结束
              this.finished = true;
            ...
            this.showTips = this.data.some(item => {
              ...
              let combtn = item.buttons.commonButtons || item.buttons.commonButton;
              let mainbtn = item.buttons.mainButtons;
              return (combtn && combtn.length > 0) || (mainbtn && mainbtn.length > 0)
            })
        ...
        .finally(() => {
          if (isEventbus) {
            this.$nextTick(() => {
              let scrollTop = this.currentClickScrollTop
              document.documentElement.scrollTop = scrollTop
              this.currentClickScrollTop = 0
            })
          }
          this.loading = false;
          this.isLoading = false;
          this.isFirst = 0;
          this.checkAll = this.checkList.length == this.data.length;
        });
    },
    onLoad() {
      /**
    ...
    onRefresh(isFist, isEventbus) {
      ...
      this.distribIds = "";
      this.start = 1;
      this.fetchData(isFist, isEventbus);
    ...
    //通用选择层验证单选还是多选函数
    checkClick(item) {
      if (this[`${this.dsfBtnAction}_checkClick`]) { // 不同批量处理按钮的自定义方法，用于拦截选中
        this[`${this.dsfBtnAction}_checkClick`](item)
        return
      }
    ...
    firstBatchSend(line) {
      // console.log(line)
      if (line.length > 1) {
        dsf.layer.toast('批量发送暂不支持多个节点同时发送')
        return
      } else {
        // console.log(this.batchSendArr)
        let receiverList = line[0].RecipientList?.Node?.map(node => {
          return {
            "type":node.Type,
            "deptId":node.DeptID,
            "deptName":node.DeptName,
            "globalShowOrder":node.GlobalShowOrder,
            "paperTypeText":"",
            "isHandleOriginal":-1,
            "isHandleOriginalText":"否",
            "userId":node.UserID,
            "userName":node.UserName,
            "secretaryId":node.secretaryId,
            "secretaryProcessMode":node.secretaryProcessMode
          }
        }) || []
        let batchSendArrPromise = this.batchSendArr.map(item => {
          return new Promise((resolve, reject) => {
            dsf.http.post('fn/mobileFlow/getFlowNode?likeAndBranch=1', {
            nextLineId: line[0].LineID,
            receiverList: JSON.stringify(receiverList),
            ...item
            }).then(res => {
              let status = res.data.data.ResponseObject[0].Status
              let successStatus = ['1000', '5000', '5100', '6000']
              if (successStatus.every(it => status.indexOf(it) == -1)) {
                reject(item.bt)
              }
              resolve()
            }).catch(err => {
        ...
        this.closeSendFile()
        dsf.layer.loading()
        Promise.all(batchSendArrPromise.map(promise => promise.catch(error => ({ error })))).then((results) => {
          const errors = results.filter(result => result?.error);
          if (errors.length > 0) {
            console.log(errors.map(result => result.error).join(','))
            dsf.layer.clear()
            dsf.layer.confirm({
              title: '以下文件发送失败',
              message: errors.map(result => result.error).join('<br>'),
              confirmButtonText: "确认",
            },
            () => {
            })
