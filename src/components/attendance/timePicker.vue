...
export default {
  ...
  computed: {
    ...
    styleDot() {
      return item => {
        return {
          // background:
          //   this.addSch(item.date) && this.judgeChecked(item.date)
          //     ? this.todayDotcolor
          //     : "",
          "background-color": this.havaSch[item.date]
          // background: this.havaSch[item.date]
        };
      };
    }
  },
  ...
  created() {
    let self = this;
    self.initTimer(self.defaultDate);
  },
  mounted() {},
  activated() {
