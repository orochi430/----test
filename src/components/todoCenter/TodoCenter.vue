<template>
<div :class="['ds-todocenter', hasBottom ? 'has-bottom' : '', {hide: hide}]">
    <van-sticky :offset-top="dsf.util.getOffsetTop()">
        <div v-if="!hide" class="search-wrap">
            <router-link to="/commonList/200827143925vksrrlGEd1ZTyiwa7tm/200826154812jQpt0dBmNzZXaNnLhTR" class="search">
                <i class="iconfont iconsousuo i1"></i>
                <span>搜索</span>
            </router-link>
        </div>
    </van-sticky>
    <van-tabs v-if="tabs.length>1" v-model="tab" :ellipsis="false" :animated="animated" :swipe-threshold="6" sticky :offset-top="isShowAppHeader?dsf.util.getOffsetTop('2.08rem'):dsf.util.getOffsetTop('0.92rem')" class="tabs" @change="tabChange">
        <van-tab v-for="item in tabs" :key="item.name" :name="item.name" :title="item.title" :badge="item.count || ''" :disabled="loading">
            <van-pull-refresh v-if="tab === item.name" v-model="isLoading" success-text="刷新成功" @refresh="onRefresh">
                <van-list :finished="finished" :finished-text="data.length ? finishedTips : ''" v-model="loading" :offset="10" @load="onLoad">
                    <DsList ref="dslist" :type="tab" :tabObj="item" :data="data" :columns="columns" :key-column="keyColumn" @mainClick="handleActionClick" @commonClick="handleActionClick" formMethod="info">
                        <template v-slot:item-extend="slotProps" v-if="tab === 'dbs'">
    ...
    <div v-if="hasBottom" class="footer">
        <DsfButtonToolbox :isFormBtn="true" :more="true" :buttons="listButtons" @mainClick="handleActionClick" @commonClick="handleActionClick" />
    </div>
...
import ButtonActions from "./TodoCenter.mixin";
...
export default {
    ...
    async mounted() {
        const tab = dsf.util.loadSessionStore("todo_center_tab");
        this.tab = tab || this.index;
    ...
    methods: {
        ...
        tabChange() {
            dsf.util.saveToSessionStore("todo_center_tab", this.tab);
            this.onRefresh();
        },
        handleActionClick() {
