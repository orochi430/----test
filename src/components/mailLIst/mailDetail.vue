<template>
    <div class="mailDetail" v-show="userdata && userdata.userName">
        <div class="header">
            <div class="title">
        ...
        <div class="orgName" v-if="navs && navs.length">{{ navs[0].name }}</div>
        <div v-for="(item, key) in showProps" :key="key" class="propBox">
          ...
          <van-icon name="chat-o" size="32" color="#1989fa" />
            ...
            <div class="infoItem disfr" v-else-if="item.prop == 'mobile'">
                <div class="info flex1 fc jsb">
                    ...
                    <span class="infoName infoColor" @click='goPhonePop(userdata.mobile)'>
                        {{ userdata.mobile | filterMobile(userdata) }}
                    </span>
                </div>
            ...
            <div class="infoItem disfr" v-else-if="item.prop == 'email'">
                <div class="info flex1 fc jsb">
                    <span class="infoText">{{ item.text }}</span>
                    <span class="infoName">{{ userdata.email | filterEmail(userdata) }}</span>
                </div>
                <div class="itemIcon disfr ac">
...
export default {
    ...
    created() {
        ...
        this.userdata = this._userdata || JSON.parse(this.$route.query.userdata)
        ...
        dsf.http.post("/fn/contacts/getMobileUserDetail", {
            ...
            if (dsf.config.projectName == "zydx") {
                if (!(await dsf.util.isMyDept(res.data.data.orgNameLv1Text))) {
                    res.data.data.mobile = "";
                    this.showProps = this.showProps.filter(item => item.prop != "mobile");
                }
            }
            ...
            if (this.navs) {
              ...
              this.deptname = this.userdata.orgName
            }
            self.$toast.clear();
        })
    },
