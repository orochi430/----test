    ...
    <div>
        <div class="moving-situation-details">
            <van-row v-if="parameters.formTemplateName == '移动表单带表格线'">
                <van-col :span="$store.state.clientType == 'mobile' ? 7 : 4">
                    <div
                        style="font-weight: 800;font-size: var(--font_size_2);padding: 0 0.12rem 0.3rem;text-align: center;">
                        流转情况</div>
                </van-col>
            </van-row>
            <div class="_content">
                <div class="_item" v-for="(item, index) in contentData" :key="index">
                    <div class="_head">
                        <img :src="dsf.url.getWebPath(item.photoImg) || defaultIcon" alt
                    ...
                    <div class="_con">
                        ...
                        <span style="color: #999" v-if="item.fduserName">发送人：{{ item.fduserName
                        }}({{ item.rDate.substring(5) }})</span>
                        <span class="_name ds_font6">
                            <span v-if="item.fduserName">接收人：</span>
                            {{ item.userName }}
                            <span v-if="item.deptName && dsf.config.commonFlow.showDeptName"> - {{ item.deptName }}</span>
                        </span>
                        <span class="_detail ds_font6"
                            :style="{ color: parameters.nodeId == item.nodeId ? '#e30000' : '' }">{{
                                getStatus(item) }}</span>
                        <p>
                            <span class="_detail ds_font6" v-if="item.content">"{{ item.content }}"</span>
                            <a v-if="item.content && dsf.config.commonFlow.showCopyOpinionBtn"
                                @click="handleClick('copyel_' + index, item.content)"
                                :class="'copyel copyel_' + index">复制意见</a>
                        </p>
                        <span class="_detail ds_font6" v-if="item.preContent">{{ item.preContent }}</span>
                        ...
                        <van-button class="cuiban"
                            v-if="item.status > 0 && dsf.config.commonFlow.showUrgingBtn && (!item.children || item.children.length == 0)"
                            type="warning" size="small" @click="cuiban(item)">催办</van-button>
                        <div v-if="item.children" class="_content _content-children">
                            <div v-for="(node, index) in item.children" :key="index" class="_item subitem">
                                <div class="_head">
                                    <img :src="dsf.url.getWebPath(node.photoImg) || defaultIcon" alt
                                        :onerror="`this.src='${defaultIcon}'`" />
                            ...
                            <van-icon v-else name="more" color="#E6A23C" /> -->
                                ...
                                <div class="_con">
                                    <span class="_titles">
                                        <i class="_title ds_font4 flex ac"> {{ node.userName }}
                                            <div class="status">{{ node.status < 0 || node.pDate.length > 0 ? "已读" : "未读" }}
                                            </div>
                                        </i>
                                ...
                                {{ getStatus(node) }}
                                    ...
                                    <p>
                                        <span class="_detail ds_font6" v-if="node.content">"{{ node.content }}"</span>
                                        <a v-if="node.content && dsf.config.commonFlow.showCopyOpinionBtn"
                                            @click="handleClick('copyel1_' + index, node.content)"
                                            :class="'copyel copyel1_' + index">复制意见</a>
                                    </p>

                                    <span class="_detail ds_font6" v-if="node.preContent">{{ node.preContent }}</span>
                                    <div class="content" v-if="node.hasHandWriteImg && node.hasHandWrite">
                                        <img style="width: 150px;" :src="dsf.url.getWebPath(node.hasHandWriteImg)" alt="">
                                    </div>
                                    <van-button class="cuiban" v-if="node.status > 0 && dsf.config.commonFlow.showUrgingBtn"
                                        type="warning" size="small" @click="cuiban(node)">催办</van-button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <van-popup v-model="showCB" position="bottom" round :style="{ height: '70%' }">
            <div class="cb_contain">
                ...
                <div class="cbinfo">
                    ...
                    <div class="cbtype">
                        <van-checkbox-group v-model="result" direction="horizontal">
                            <van-checkbox icon-size="16px" :name="item.codeValue" v-for="item in typeList"
                                :key="item.codeValue">{{ item.codeName }}</van-checkbox>
                        </van-checkbox-group>
                    </div>
                ...
                <div class="btoms">
                    <van-button type="primary" size="small" @click="doCBAction">催办</van-button>
                </div>
            </div>
        </van-popup>
...
export default {
    ...
    methods: {
        cuiban(node) {
            ...
            dsf.http.post(`ctrl/flowmonitor/getUrgeRemindConfig`, {
                infoId: infoId,
                moduleId: moduleId
            }).then(({
                data
        ...
        doCBAction() {
            ...
            dsf.http.post(`ctrl/flowmonitor/submitUrge`, {
                moduleId: moduleId,
                docid: docid,
                bt: bt,
            ...
            handler(nv, ov) {
                ...
                if (nv.passedNodes) {
                    ...
                    Object.keys(nodeGroup).forEach(key => {
                        ...
                        item.userName = "接收人：" + item.children.map(obj => obj.userName).join("、")
