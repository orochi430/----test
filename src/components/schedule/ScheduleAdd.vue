...
<div class="ds-schedule-add">
    ...
    <van-form @submit="onSubmit('',1)">
        ...
        <van-cell-group>
            ...
            <van-field v-model="form.address" name="address" left-icon="i iconfont icondingwei"  placeholder="请输入日程地点" />
        </van-cell-group>
        <van-cell-group>
...
export default {
    ...
    data() {
        ...
        return {
            ...
            formId: "180719140247z1QE99WtCq0pw08nLdU",
            info_id: "",
            form: {
                schedule: "",
                isAllDay: false,
    ...
    methods: {
        ...
        onSubmit(callback, num) {
            ...
            this.maininfo["C-RC-0007"].value = this.form.address;
            ...
            if(Date.parse(filterTime(this.sdate))>Date.parse(filterTime(this.edate))){
                dsf.layer.toast("开始时间不能晚于结束时间")
                return
            }
            Object.keys(this.maininfo).forEach((obj, index) => {
                newObj[obj] = {
            ...
            dsf.http.post("/fn/xform/save", params).then(function (data) {
                ...
                if (data.data.type == 'success') {
                    ...
                    that.$toast.fail(data.data.message||"提交失败,请重新提交");
                }
                // that.$toast.success("保存成功")
        ...
        initData() {
            ...
            //       });
            //   }
            let url = "/fn/xform/data?moduleId=180719094152MnF6C2hEPtqIvhjJIxo&pk=";
            if (this.info_id) {
                url =
                    "/fn/xform/data?moduleId=180719094152MnF6C2hEPtqIvhjJIxo&pk=" +
                    this.info_id;
            }
            let that = this;
            ...
            dsf.http.get(url, {}).then(function (args) {
                if (args && args.data && args.data.main) {
                    ...
                    if (that.maininfo["C-RC-0007"]) {
                        that.form.address = that.maininfo["C-RC-0007"].value
                    }
                    ...
                    console.log(that.maininfo["C-RC-0008"])
        ...
        doFileRead(file, filetype) {
            ...
            dsf.http
                ...
                .then(function (data) {
                    console.log(data)
                    console.log(self.attach_1)
                    if(data.data.result){
                        self.attach_1.push({
                            type:data.data.data.nrType,
                            id:data.data.data.fileId,
                            moduleId: self.moduleId,
                            mid:self.moduleId,
                            title:data.data.data.fileName,
                        })
                        self.$toast.clear()
                    }else{
        ...
        removeFile(item) {
            ...
            dsf.http
                ...
                .then(function (data) {
                    that.attach_1.forEach((itm,index)=>{
                        console.log(item.id)
                        if(item.id==itm.id){
