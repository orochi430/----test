  ...
  <div class="ds-meeting-book">
    ...
    <van-popup v-model="showStartTimePicker1" position="bottom">
      <van-datetime-picker
        ...
        type="date"
        @confirm="onConfirm($event, 'startTime')"
        @cancel="showStartTimePicker1 = false"
      />
    </van-popup>
    <van-popup v-model="showEndTimePicker1" position="bottom">
      <van-datetime-picker
        ...
        type="date"
        @confirm="onConfirm($event, 'endTime')"
        @cancel="showEndTimePicker1 = false"
      />
    </van-popup>
    <van-form @submit="onSubmit('',1)">
      <van-cell-group>
        <van-field
          class="meeting-textarea files"
          name="meeting"
          label="会议"
          left-icon="i iconfont iconricheng"
        >
          <template #input>
            <textarea
              class="textr"
              style="text-align:left; height:72px !important;"
      ...
      <van-cell-group>
        ...
        <div class="time-Range">
          <div
            :style="{opacity: form.startTime ? 1 : 0.7 }"
            class="time-Range-item time-Range-start"
            @click="chooseStartTime"
          >
            <div class="time-Range-day">{{ form.startTime | dateFilter }}</div>
            <div
              v-show="!form.isAllDay"
              class="time-Range-time"
            >{{ form.startTime | timeFilter('开始时间') }}</div>
          </div>
          <div
            :style="{opacity: form.endTime ? 1 : 0.7 }"
            class="time-Range-item time-Range-end"
            @click="chooseEndTime"
          >
            ...
            <div
              v-show="!form.isAllDay"
              class="time-Range-time"
            >{{ form.endTime | timeFilter('结束时间') }}</div>
          </div>
        </div>
      </van-cell-group>
      <van-cell-group>
        <van-field
          ...
          name="address"
          left-icon="i iconfont icondingwei"
          right-icon="i iconfont iconaddress-dark"
          placeholder="请输入会议地点"
        />
      </van-cell-group>
      <!-- <van-cell-group>
      ...
      <van-cell-group>
        <van-field
          ...
          name="address"
          readonly
          left-icon="i iconfont iconhuiyi"
          placeholder="选择会议主持"
          is-link
          @click="selectUser(0)"
        />
      ...
      <van-cell-group>
        <van-field
          ...
          name="address"
          readonly
          left-icon="i iconfont iconyaoqingcanyuren"
          placeholder="选择参会领导"
          is-link
          @click="selectUser(1)"
        />
      ...
      <van-cell-group>
        <van-field
          ...
          name="address"
          readonly
          left-icon="i iconfont iconfasongren_danwei"
          placeholder="选择参会单位"
          is-link
          @click="selectUser(2)"
        />
      ...
      <van-cell-group>
        <van-field
          ...
          name="address"
          readonly
          left-icon="i iconfont iconyidao"
          placeholder="选择参会人员"
          is-link
          @click="selectUser(3)"
        />
      ...
      <van-field style="margin-top:15px" input-align="right" name="uploader">
        <template #label>
          <span>
            <label class="mast-input">*</label>附件：
          </span>
        </template>
        <template #input>
          <div></div>
      ...
      <div class="footer">
        <van-button type="info" native-type="submit()">提交审核</van-button>
      </div>
    </van-form>
    <van-popup
    ...
    >
      <selectPerson
        v-if="showPerson"
        :extra="{tabs:[{key:'self_org'}]}"
        :type="meeting_type"
        @getSelectUser="getSelectUser"
        @cancePerson="cancePerson"
      ></selectPerson>
    </van-popup>
    ...
    >
      ...
      <send-workflow
        ...
        ref="sendFile"
        :form-data="dataValue"
        @close="showFlowSend = false;showPopup=false"
        @golist="onRefresh"
      />
    </van-popup>
  </div>
...
export default {
  ...
  methods: {
    ...
    onRefresh() {
      this.$store.commit("refreshRouteByReg", [
        "/meetingCenter",
        "/todocenter",
        "/",
      ]);
    ...
    onSubmit(callback, num) {
      ...
      this.maininfo["A0005"] = {};
      this.maininfo["A0005"].value = "会议申请";
      ...
      this.maininfo["c-meetingtz-030"].value = this.form.address;
      ...
      //参会部门
      newObj["c-meetingtz-028"] = {};

      let ids = this.chdw_list.id ? this.chdw_list.id.split(",") : [];
      let curIds = "";
      ids.forEach((obj, index) => {
        let id = "4::" + obj.split("_")[1];
        if (curIds == "") {
          curIds = id;
        } else {
          curIds += "," + id;
    ...
    selectUser(index) {
      ...
      this.meeting_type = this.cur_index == 2 ? 3 : 0;
      console.log("选人");
    },
    closePerson() {
    ...
    postData() {
      ...
      dsf.http
        .post("fn/meeting/meetingRoomJson", {
          date:
            this.$route.query.date || dsf.date.format(new Date(), "yyyy-mm-dd"),
        })
        .done((res) => {
          if (res.code + "" === "200") {
